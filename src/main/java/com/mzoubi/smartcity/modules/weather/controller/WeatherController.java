...
public class WeatherController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<Page<WeatherDto>>> getAllWeather(
        ...
        Page<WeatherDto> weatherList = weatherService.getAllWeather(pageRequest);
        return ResponseEntity.status(HttpStatus.OK)
                .body(ApiResponse.success("Fetched all weather", weatherList, HttpStatus.OK));
    }

    @GetMapping("/city/{cityId}")
    public ResponseEntity<ApiResponse<Page<WeatherDto>>> getWeatherByCity(
            ...
            @RequestParam(value = "sort-direction", required = false) String sortDirection) {
        PageRequest pageRequest = PaginationUtils.createPageRequest(pageNumber, pageSize, sortBy, sortDirection,ALLOWED_FIELDS);
        Page<WeatherDto> cityWeather = weatherService.getWeatherByCityId(cityId,pageRequest);
        return ResponseEntity.status(HttpStatus.OK)
